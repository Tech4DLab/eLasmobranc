<p align="center">
  <img src="./images/cimar.jpg" alt="e-Lasmobranc Banner" width="1000"/>
</p>

<h1 align="center">🦈 e-Lasmobranc 🐟</h1>
<h3 align="center">🏛️ University of Alicante 🏛️</h3>

<p align="center">
  <strong>
    Welcome to the official repository of the Tech4Diet Research Lab at the University of Alicante.  
    We focus on applied research in Artificial Intelligence, particularly in areas such as machine learning and computer vision.  
    The repository includes reproducible code, datasets, models and detailed documentation to facilitate research in marine monitoring and conservation.
  </strong>
</p>

<p align="center">
  
  <a href="https://fundacion-biodiversidad.es/en/programa_pleamar/e-lasmobranc-new-technologies-and-advances-in-the-knowledge-of-elasmobranchs-from-the-spanish-east/">
    <img src="https://img.shields.io/badge/Official_Site-eLasmobranc-007BFF?style=for-the-badge&logo=google-chrome&logoColor=white"/>
  </a>

  <a href="https://github.com/Tech4DLab/eLasmobranc-Dataset">
    <img src="https://img.shields.io/badge/Dataset-eLasmobranc-6f42c1?style=for-the-badge&logo=zenodo&logoColor=white"/>
  </a>

  <a href="MODEL">
    <img src="https://img.shields.io/badge/Model-Identification-orange?style=for-the-badge"/>
  </a>

  <a href="YOUR_APP_LINK">
    <img src="https://img.shields.io/badge/Web_App-eLasmobranc-2196F3?style=for-the-badge&logo=streamlit&logoColor=white"/>
  </a>

</p>

---

## 🔥 News

- **February 2026** – 🎤 *Joint presentation of results with related elasmobranch projects (TIRRAQUI, e-Lasmobranc and GUITAR-HERO).*  
  Participation in the official results workshop bringing together multiple elasmobranch-focused initiatives. During the event, both the e-Lasmobranc Dataset and the identification application were presented.  
  🔗 Results workshop: https://www.programapleamar.es/actualidad/agenda/jornada-de-presentacion-de-resultados-de-tirraqui-e-lasmobranc-y-guitar-hero

- **February 2026** – 🗞️ *First public release of the e-Lasmobranc Dataset!*  
  Publication of the first public version of the e-Lasmobranc Dataset on Zenodo, providing curated image collections to support automated elasmobranch identification and downstream AI research.  
  🔗 Dataset: https://zenodo.org/records/18549737

- **February 2026** – 🌐 *Release of the e-Lasmobranc identification application.*  
  Official distribution of the identification application, accompanied by detailed installation instructions, examples, and supporting resources.

<details>
<summary><strong>📰 View remaining news</strong></summary>

<br>

- **January 2026** – 💶 *e-Lasmobranc project funded by the Pleamar Programme of Biodiversity Foundation.*  
  The e-Lasmobranc project was officially selected in the Pleamar call for proposals, co-financed by the Fondo Europeo Marítimo, de Pesca y Acuicultura (FEMPA), supporting research into new technologies and advances in knowledge of elasmobranch species in southeastern Spain.  
  🔗 Call for proposals: https://fundacion-biodiversidad.es/convocatorias_pleama/convocatoria-de-ayudas-del-programa-pleamar-2025/

- **October 2025** – 📄 *Paper accepted: “Zero-Shot Elasmobranch Classification Informed by Domain Prior Knowledge”.*  
  Acceptance of the paper presenting an advanced zero-shot identification framework integrating domain-specific prior knowledge, representing a near-final version of the elasmobranch identification model.  
  🔗 DOI: https://doi.org/10.3390/make7040146

- **September 2025** – 📘 *Publication of the “Image Capture and Structuring Protocol for Elasmobranch Species Differentiation”.*  
  Release of a standardized protocol for image acquisition and organization to support consistent species differentiation and dataset construction.
  🔗 Protocol: https://agris.fao.org/search/en/providers/125030/records/68b6c5a268d9e680670075b8

- **September-November 2025** – 🎤 *Early acceptance and preliminary elasmobranch identification model presented at UCAMI 2025.*  
  Presentation of early results on AI-based elasmobranch identification, marking the first public disclosure of initial methodological approaches, laying the groundwork for subsequent zero-shot and knowledge-informed approaches.

- **January 2025** – 🚀 *Official start of e-Lasmobranc Phase 1 (e-Lasmobranc 1).*  
  After an initial organization and planning period, the first phase of the e-Lasmobranc project formally began, focusing on dataset preparation, preliminary identification models and the definition of domain-informed methodologies.

</details>




---

## 🗺️ Roadmap

<details>
<summary>📘 <strong> Dataset Development and Expansion</strong></summary>

- [x] Study of the current situation  
- [x] Identification of reliable data sources  
- [x] Definition of an acquisition and curation protocol  
- [x] Development of the first official dataset version (minimum target: 1000 samples) — e-Lasmobranc 1  
- [ ] Development of the second official dataset version with additional species (minimum target: 2000 samples) — e-Lasmobranc 2  

</details>

<details>
<summary>📗 <strong> Model Development and Refinement</strong></summary>

- [x] State-of-the-art review  
- [x] Development of an initial identification model  
- [x] Definition of the definitive model version 1 — e-Lasmobranc 1  
- [ ] Exploration of solutions and adaptations after problem understanding  
- [ ] Development of an extended model version 2 — e-Lasmobranc 2 

</details>

<details>
<summary>📕 <strong> Application and Resource Development</strong></summary>

- [x] Design and development of mockups 
- [x] Web application for dataset exploration and analysis — e-Lasmobranc 1  
- [ ] Development of the mobile application
- [ ] Deployment of the web application and the mobile application  
- [ ] Model updates within deployed applications — e-Lasmobranc 2 

</details>

## 📄 Publications

- **Beviá-Ballesteros, I., Jerez-Tallón, M., Aranda-Garrido, N., Saval-Calvo, M., Abel-Abellán, I., & Fuster-Guilló, A. (2025).**  
  *Zero-Shot Elasmobranch Classification Informed by Domain Prior Knowledge.*  
  Machine Learning and Knowledge Extraction, 7(4), 146.  
  https://doi.org/10.3390/make7040146

- **Beviá Ballesteros, I., Jerez Tallón, M., Sánchez Sos, B., Aranda Garrido, N., Abel Abellán, I., Antón Linares, I., Saval Calvo, M., Giménez Casalduero, F., Azorín López, J., & Fuster Guilló, A. (2025).**  
  *Informed Zero-Shot Elasmobranch Classification Using Expert Knowledge and Illustrations.*  
  UCAMI 2025 – Pending submission.

- **Abel, I., Antón, I., Aranda Garrido, N., Azorín López, J., Beviá, I., Fuster-Guilló, A., … & Saval-Calvo, M. (2025).**  
  *Protocolo de Captura y Estructuración de Imágenes para la Diferenciación de Especies de Elasmobranquios.*


## 👥 Research Team

| Name | Role | Contact |
|------|------|---------|
| Ismael Beviá Ballesteros | PhD Student / Researcher | ismael.bevias@ua.es |
| Nieves Aranda Garrido | Researcher | – |
| Isabel Abel Abellán | Researcher | – |
| Irene Antón Linares | Researcher  | – |
| Dr. Marcelo Saval Calvo | Supervisor | m.saval@gcloud.ua.es |
| Dr. Jorge Azorín López | Researcher | jazorin@ua.es |
| Dr. Andrés Fuster Guilló | Project Leader / Supervisor | fuster@ua.es |
| Dr. Francisca Giménez Casalduero | Project Leader | – |

## 🤝 Acknowledgments

This research was funded by the eLasmobranc project, which is developed with the collaboration of the Biodiversity Foundation of the Ministry for Ecological Transition and the Demographic Challenge, through the Pleamar Programme, and is co-financed by the European Union through the European Maritime, Fisheries and Aquaculture Fund.


<p align="center">
  <img src="./images/footer6.png" width="1200"/>
</p>
